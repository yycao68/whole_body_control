...
def get_contact_consistent_inverse(M, Jc, reg=1e-3):
    """M̄⁻¹ = M⁻¹(I − Jc^T Λc Jc M⁻¹)   (Eq. 9).
    reg controls Tikhonov regularization that prevents singularities when
    the contact Jacobian nearly spans the arm task space.
    """
    M_inv = np.linalg.inv(M + reg * np.eye(M.shape[0]))
    if Jc.shape[0] == 0:
        return M_inv
    nc = Jc.shape[0]
    Lambda_c = np.linalg.inv(Jc @ M_inv @ Jc.T + reg * np.eye(nc))
    Pc = np.eye(M.shape[0]) - Jc.T @ Lambda_c @ Jc @ M_inv
    return M_inv @ Pc.T
...
class WBCController:
    ...
    def compute(self, data, F_arm_mpc, contact_mask=None,
        ...
        # ── Arm task torque via J^T F (Level 3) ───────────────────────────
        # Extract arm-only columns of J_arm (indices 14,15,16)
        arm_dofs  = [ids[k] for k in self._ARM_DOF_KEYS]
        arm_qadrs = [ids[k] for k in self._ARM_QADR_KEYS]
        J_arm_cols = J_arm[:, arm_dofs]          # 3×3 arm-only Jacobian
        tau_arm_task = J_arm_cols.T @ F_arm_mpc  # (3,) arm joint torques
