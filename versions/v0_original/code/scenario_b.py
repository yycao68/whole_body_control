...
Scenario B: Double-Support Stance with Periodic Contact-Transition Shocks
 ...
 - At each spike onset the Kalman is notified (mode key changes), allowing
   the covariance-inflation protocol (D7) to widen the filter and re-estimate
   d_hat faster than D6 (no inflation)
...
Why this models the reviewer's concern:
  ...
  the shock component, which is directly testable in a stable stance.
  D7 must outperform D6 here, and D2 (PI) must show inferior transient
  recovery because integral action cannot pre-compensate for a
...
from simulation.controllers.gait import StanceBalance
...
def run_controller(ctrl_name, ctrl_cfg):
    ...
    if ctrl_cfg.get('use_mpc'):
        mpc = ImpedanceMPC(N=20, dt=CTRL_DT,
                           Q=np.diag([60000., 60000., 60000., 60., 60., 60.]),
                           R=0.01 * np.eye(3), F_max=80.0)
        La = 0.20 * np.eye(3) if use_cc else 2.00 * np.eye(3)
    ...
    while t < T_TOTAL and step < n_steps:
        ...
        # Detect mode switch onset
        if switch_idx != prev_switch_idx and switch_idx > 0:
            mode_switch_times.append(t)
            if kalman is not None:
                alpha = ctrl_cfg.get('inflate_alpha', 4.0)
                if alpha > 1.0:
                    kalman.inflate_covariance(alpha)
            prev_switch_idx = switch_idx

        # Compute arm force
        La_cur = 0.20 * np.eye(3) if use_cc else 2.00 * np.eye(3)
        if mpc is not None:
            d_hat = None
            if kalman:
