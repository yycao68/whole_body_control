...
Flat & Impedance & 4.782 & 52.204 & 66.89 & 10 \\
 & Nominal MPC & \textbf{4.139} & 11.434 & 41.64 & 0 \\
 & ID-MPC & 4.497 & \textbf{10.636} & \textbf{39.74} & 0 \\
Depression & Impedance & 5.839 & 48.472 & \textbf{66.36} & 10 \\
 & Nominal MPC & 5.823 & 47.479 & 68.59 & 10 \\
 & ID-MPC & \textbf{5.799} & \textbf{42.557} & 68.66 & 10 \\
Obstacle & Impedance & \textbf{3.989} & 11.564 & \textbf{34.32} & 0 \\
 & Nominal MPC & 4.510 & 11.434 & 41.10 & 0 \\
 & ID-MPC & 4.568 & \textbf{10.636} & 39.59 & 0 \\
Rough & Impedance & 6.072 & 48.900 & \textbf{64.44} & 10 \\
 & Nominal MPC & 6.149 & 48.135 & 66.64 & 10 \\
 & ID-MPC & \textbf{6.033} & \textbf{43.564} & 66.66 & 10 \\
\bottomrule
\end{tabular}
\end{table*}
...
On flat ground, nominal MPC and ID-MPC complete all ten 15 s trials, whereas impedance falls in all ten. ID-MPC has a lower peak than nominal MPC (10.636 versus 11.434 mm) but a higher RMS error (4.497 versus 4.139 mm), locating its benefit in peak attenuation rather than flat-ground RMS tracking. On the valid future obstacle, all controllers complete all trials. ID-MPC reduces median peak error by 7.0\% relative to nominal MPC, while its RMS is higher (4.568 versus 4.510 mm) and impedance has the lowest RMS. On the depression and rough sequence every controller falls, typically after reaching the terrain change. The smaller ID-MPC peak in those truncated records characterizes pre-fall attenuation, not fall prevention. Figure 5 shows representative time histories alongside the aggregate outcomes.
...
\midrule
Lateral, double support & 60.08 / 59.59 / \textbf{51.30} & $-13.9\%$ & -- / -- / -- & 10 / 10 / 10 \\
Lateral, single support & 15.83 / 16.00 / \textbf{12.37} & $-22.7\%$ & 0.764 / 0.754 / \textbf{0.278} & 0 / 0 / 0 \\
Forward, double support & 19.91 / 19.39 / \textbf{18.19} & $-6.2\%$ & 0.752 / 0.728 / \textbf{0.661} & 0 / 0 / 0 \\
Forward, single support & 15.74 / 15.82 / \textbf{13.71} & $-13.3\%$ & -- / -- / -- & 0 / 0 / 0 \\
\bottomrule
\end{tabular}
\end{table*}

ID-MPC reduces median peak error relative to nominal MPC in all four conditions. The largest recoverable reduction occurs for the lateral single-support push, where peak error decreases by 22.7\% and median recovery time decreases from 0.754 to 0.278 s. Forward double-support recovery changes from 0.728 to 0.661 s. All controllers fall under the lateral double-support push, and none satisfies the recovery-band definition for the forward single-support push despite zero falls. The measured effect is therefore phase- and direction-dependent attenuation within this protocol.
...
Timing is measured on a general-purpose workstation under a standard operating system in an unoptimized Python implementation; it is a prototype measurement on a non-real-time host. Across terrain trials, the median of per-trial WBC medians is 3.82 ms and the median WBC p99 is 10.68 ms; the median deadline-miss fraction is 100\% against the simulated 2 ms period. The corresponding MPC values are 1.68 and 2.77 ms, with a 0.20\% median miss fraction against the simulated 10 ms period. Observed maxima reach 37.94 ms for WBC and 24.93 ms for MPC. The 500 Hz and 100 Hz values are simulation schedules, not hard real-time execution claims; compiled implementation and real-time scheduling are required for deadline validation.
...
The authoritative terrain and push JSON files are stored with the code. The supplied evidence gate passes only if they use schema version 2, contain the exact three-controller paired matrices, record the frozen residual conditioning, show valid future obstacle contact, gate every push on the requested measured support phase, report zero QP fallback, and match the hashed no-root-assist video. The numerical, configuration, figure, and timing checks pass in the current checkout; the complete release gate additionally requires regeneration of the missing MP4. A machine-readable verification record accompanies the data.

\section{Operating Envelope and Validation Scope}\label{sec:limitations}

The evaluation isolates interaction compensation by holding the body/foot reference and contact schedule fixed. ID-MPC therefore complements terrain-aware planning: if a foothold or timing choice makes the shared reference infeasible, residual correction retains the existing support strategy rather than creating a new one. This operating boundary is visible in the depression and rough-terrain trials; online foot placement and timing adaptation address the planning side of that boundary.
...
The reported study uses one Unitree G1 model, selected CoM and roll/pitch tasks, four terrain profiles, one push magnitude, and ten paired seeds per condition. MuJoCo torque actuation approximates hardware sensing, transmission, delay, and impact effects. The MPC misses its simulated 10 ms period in a small fraction of cycles, while the Python whole-body QP misses the 2 ms period systematically. These are simulation schedules; deployment requires a compiled implementation, real-time scheduling, and filtered state estimation.

Within this scope, the experiments establish repeatable peak-error attenuation under a shared plan and realizer. Broader terrain envelopes, push-magnitude sweeps, longer-distance locomotion, and coupling the interaction residual to step adaptation define separate validation axes for the interface.
...
The 240-trial torque-level evaluation used paired seeds, verified future terrain contact, measured-phase push gating, and identical planning and realization infrastructure across controllers. ID-MPC completed every flat and raised-obstacle trial, reduced obstacle peak CoM error by 7.0\% relative to nominal MPC, and reduced median peak response in all four push conditions by 6.2--22.7\%. In the lateral single-support case, median recovery changed from 0.754 s for nominal MPC to 0.278 s for ID-MPC, characterizing the benefit of conditioning an already-observable interaction residual. All controllers fall on the tested depression, rough-terrain, and lateral double-support-push cases, which defines the observed operating boundary of the fixed-plan stack.

The results support interaction dynamics as a reusable layer that composes with locomotion planning. A combined system can use the interaction residual to regulate the current task while online foot placement and timing adaptation change the reference when the support strategy must change. Compiled real-time execution and hardware experiments form the corresponding implementation and physical-validation stages.
