...
def load() -> dict:
    data = json.loads((RESULTS / "uneven_ground_benchmark.json").read_text())
    if data.get("schema_version") != 2:
        raise RuntimeError("refusing to plot legacy terrain artifact; regenerate schema 2")
    if tuple(data.get("metadata", {}).get("controllers", ())) != CONTROLLERS:
        raise RuntimeError("terrain artifact is not the exact publication controller matrix")
    return data
...
def tracking_figure(data: dict) -> None:
    ...
    for ci, (controller, label, color) in enumerate(zip(CONTROLLERS, LABELS, COLORS)):
        ...
        offset = (ci - (len(CONTROLLERS) - 1) / 2.0) * width
...
def prediction_figure(data: dict) -> None:
    ...
    # confounded by the different closed-loop commands of the three controllers.
...
def timeseries_figure() -> None:
    ...
    axes[2].set_ylabel("Aggregate task-acc. residual")
...
def timing_figure(data: dict) -> None:
    trials = [t for t in data["trials"] if t["controller"] in CONTROLLERS]
    wbc_median = np.asarray([t["timing"]["wbc"]["median_ms"] for t in trials])
    wbc_p99 = np.asarray([t["timing"]["wbc"]["p99_ms"] for t in trials])
    mpc_median = np.asarray([t["timing"]["mpc"]["median_ms"] for t in trials])
    mpc_p99 = np.asarray([t["timing"]["mpc"]["p99_ms"] for t in trials])
