...
\title{Interaction Dynamics: A Configuration-Invariant Predictive Model for Humanoid Locomotion under Terrain and External Disturbances}
\author{Yongyan Cao%
\thanks{Y. Cao. E-mail: yongyancao@gmail.com.}}
\markboth{Manuscript}{Cao: A Configuration-Invariant Predictive Model for Humanoid Locomotion}
\maketitle
...
\begin{enumerate}
\item \textbf{Interaction as a predictive state on a fixed shared model.} We show (Theorem~1) that for the normalized requested-task coordinates the selected body tasks share one exact-ZOH transition pair that is provably invariant across gait phase, terrain, and contact mode, and that terrain, contact timing, external force, and realization error all enter a single interaction residual $\deff$ rather than the model. A non-vacuity argument, confirmed by the experiments, establishes that this is a falsifiable modeling claim rather than a free relabelling.
...
Let a controlled task have error coordinate $e$ of relative degree $r$, and let $\mathcal M$ be a set of contact modes for which Assumption~1 holds with the \emph{same} requested coordinate. Then for every mode $\rho\in\mathcal M$ the realized error obeys the canonical model (1),
\begin{equation}
...
\begin{proof}
Substituting the feedforward $F^{\rm act}=\mu_p+M_p(\ddot y_d+a_e)+\delta$ into the constrained dynamics $M_p\ddot y+\mu_p=F^{\rm act}+F^{\rm ext}$ cancels $\mu_p$, and left-multiplying by $M_p^{-1}$ gives $\ddot e=\ddot y-\ddot y_d=a_e+M_p^{-1}(F^{\rm ext}+\delta)=a_e+\deff$. The input-to-error map $a_e\mapsto\ddot e$ is therefore the identity \textbf{in every mode $\rho\in\mathcal M$}, independent of $M_p$, $\mu_p$, and $\rho$. Its exact-ZOH sampling is the order-$r$ integrator pair, whose entries are polynomials in $\Delta t$ only (for $r=2$, $A_d=\big[\begin{smallmatrix}I&\Delta t\,I\\0&I\end{smallmatrix}\big]$, $B_d=\big[\begin{smallmatrix}\frac12\Delta t^2I\\\Delta t\,I\end{smallmatrix}\big]$). No mode-dependent quantity can appear in $(A_d,B_d)$ because none appears in the map it discretizes; every such quantity is absorbed, \emph{by construction}, into $F^{\rm act}$ --- hence into the recovery map, the admissible set, and $\deff$.
\end{proof}
