...
def _prediction_rmse(log: dict, dt: float) -> dict:
    ...
    d = np.asarray(log["effective_estimate"])
...
def run_trial(controller: str, terrain: str, seed: int, duration: float = 4.0,
    ...
    # Both default to off, keeping the frozen benchmarks byte-identical.
    ...
    g = {"step_length": 0.03, "n_steps": 8, "step_time": 0.80,
         "double_support_time": 0.55, "settle_time": 1.0, "lateral_zmp_scale": 0.85}
    if gait:
        g.update(gait)
    ...
    for k in range(n):
        ...
        if k - last_wbc_k >= round(WBC_DT / SIM_DT):
            ...
            if k - last_mpc_k >= round(mpc_solve_dt / SIM_DT):
                ...
                if controller == "impedance":
                    ...
                    candidate = mpc.solve(state, d_control)
                else:
                    d_control = np.sign(estimate_interaction) * np.clip(
                        np.abs(estimate_interaction) - 0.30, 0.0, 0.50
                    )
                    candidate = mpc.solve(state, d_control)
                ...
                correction = correction + np.clip(candidate - correction, -0.70, 0.70)
            ...
            if np.all(np.isfinite(realizer.last_qdd)):
                realized_com_acc = Jcom @ realizer.last_qdd
        ...
        log["realization_residual"][k] = realized_error_acceleration - correction
...
def aggregate(trials: list[dict]) -> dict:
    ...
    for terrain in TERRAINS:
        for controller in CONTROLLERS:
            ...
            for metric in ("com_xyz_rms_mm", "com_xyz_peak_mm", "roll_pitch_rms_rad",
    ...
    return {"schema_version": 1, "trials": trials, "cells": cells}
