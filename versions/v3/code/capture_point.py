...
EXPERIMENTAL / NOT ENABLED IN THE REPORTED BENCHMARKS.  It is a future-work
software interface for embedding the interaction layer in a locomotion
controller.  An extended-walking diagnostic showed the failure is not a
tracking or swing-realization problem (commanded/realized touchdowns agree to
~5 mm and the reference DCM is tracked to ~16 mm); the shared nominal lateral
gait reference itself puts the DCM ~135 mm from the stance foot each step,
saturating the step-width correction.  Making this layer robust therefore
requires a capturability-aware lateral gait reference and CoP regulation, which
is out of scope for the present paper; ``run_trial`` leaves it disabled by
default so the reported comparisons are byte-identical.
"""
