...
Four rates, slowest at the top:
  ...
  * 100 Hz INTERACTION LAYER -- an augmented random-walk estimator produces the
    effective residual d_eff and its horizon sequence, and the interaction MPC
    picks a task-acceleration correction v on the fixed double integrator (A_d,B_d).
...
def main():
    ...
    box(ax, 0.20, 0.56, 0.2, 0.17, "Interaction estimator",
        [r"augmented random-walk KF",
         r"$\hat d_{\rm eff}=d_{\rm int}+d_{\rm real}+d_{\rm mod}$",
         r"horizon  $\hat d_{k+i|k}=\hat d_{k|k}$ (10)"], C_MPC, E_MPC, lead=0.044)
    box(ax, 0.465, 0.56, 0.23, 0.17, "Interaction-Dynamics MPC",
        [r"fixed $(A_d,B_d)$ double integrator",
         r"$\min\ \sum\|x_j\|_Q^2+\|v_j+\hat d_k\|_R^2$",
         r"task-acceleration correction $v$"], C_MPC, E_MPC, lead=0.044)

    # ---- realizer ------------------------------------------------------
    box(ax, 0.3, 0.310, 0.36, 0.150,
        "Whole-body inverse-dynamics / contact QP",
        [r"realizes $\ddot y_d+v$;  hard multibody, contact,",
    ...
    # MPC -> realizer (correction v)
    arrow(ax, (0.60, 0.565), (0.6, 0.460), E_MPC)
    note(ax, 0.67, 0.5, r"correction $v$ (100 Hz)", E_MPC, fs=8.0, style="normal")
