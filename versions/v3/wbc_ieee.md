...
No robot-specific dynamics enter the state transition or the cost; the only footprint of the physics is the frozen feasible set $\widehat{\mathcal U}_{b,k}$, while exact contact-force, friction, CoP, and torque feasibility — and the residual $r_b$ — are produced instantaneously by the realizer (22). The task port (Section V) is organized identically, with its own feasible set $\widehat{\mathcal U}_{t,k}$.
...
H1 is the empirical test of Theorem 1 and the centerpiece of the evaluation: the predictor is invariant while everything robot-specific lives in recovery. Five measurements make this precise:
...
**Table I.** H2, representation level and full torque-actuated G1 realizer. The observer removes the steady-state offset in both settings and neither port falls.
