...
The *evaluated* realizer is a practical weighted-objective variant of (22), not its exact form. Eliminating the explicit slacks in favor of penalizing their residuals is exact; the remaining differences are implementation choices. The body request is imposed as a three-dimensional CoM-acceleration objective $\|J_{\rm com}\ddot q-(\ddot c_d+u_c)\|^2$ rather than the six-dimensional centroidal-wrench objective, and torso orientation is regulated through a separate angular-acceleration penalty rather than the centroidal angular-momentum request $u_\theta$; the task request is a soft acceleration objective; and rigid-contact consistency is a hard equality only in the non-virtual-foot configuration, otherwise a weighted objective. The quantity stored in the benchmark as the realizer residual is the rigid-body dynamics-equality residual, not the body/task realization slacks $s_W,s_t$ of (22), which are not separately logged. We therefore present the reported results as those of this practical variant rather than of the exact QP (22).
