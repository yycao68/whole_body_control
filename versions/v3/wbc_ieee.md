...
Humanoid robots regulate two physical interfaces at once: the feet exchange forces with the environment for balance and locomotion, the hands exchange forces with people, tools, and objects for a task. Existing stacks assign these to different mathematical objects — a centroidal or single-rigid-body MPC plans contact forces, a whole-body QP maps them to joint commands, and an impedance-like controller regulates the hand. This is practical, but hides that both interfaces are manifestations of the same interaction-dynamics problem.

The question here is not whether another whole-body architecture can be assembled, but whether floating-base manipulation admits the same normalized interaction-dynamics representation derived for fixed-base systems in [1]. If so, balance and manipulation are not unrelated modules but two ports of one predictive representation, with robot-specific mechanics appearing only when the normalized commands are recovered as physical wrenches. This separation carries a broader payoff for the emerging *physical-AI* stack: learned policies and world models increasingly generate whole-body *intent*, but intent is not execution. A robot-independent interaction predictor is a natural target for that intent — the generator says *what* interaction to perform, and the realizer turns it into safe, constraint-respecting *physical execution*. We return to this in Sections II and XII.
...
The resulting error model is the interaction-dynamics backbone of [1]. The floating-base case is nontrivial because contact geometry, support changes, friction, center-of-pressure limits, actuator saturation, and arm–body reactions all affect whether a normalized acceleration can be realized. The central claim is therefore a prediction–realization separation: only interaction dynamics are predicted over a horizon, while the full robot dynamics are used at the current sample to project interaction requests onto the feasible whole-body set — an instantaneous feasibility projection, not a second prediction model.

This yields, secondarily, a dual predictive controller. Both ports share the same exact-ZOH predictor — the task port a double integrator, the body port a double integrator on the CoM error plus a first-order integrator on the centroidal angular momentum — while the body port recovers its command as a centroidal wrench and the task port as a contact-consistent task wrench. A whole-body realizer then enforces the rigid-body dynamics and feasibility constraints. The architecture thus follows from the representation, not the reverse.

The contributions follow this separation. First, we formulate floating-base balance and manipulation as two interaction ports instantiating the normalized model of [1]. Second, we derive the centroidal and contact-consistent task recoveries and show that mass, centroidal inertia, contact mode, and task inertia do not alter the prediction matrices. Third, we localize contact geometry, friction, center-of-pressure, actuator limits, and whole-body dynamics to recovery, clarifying where floating-base constraints enter. Fourth, we realize the representation as a split or coupled dual-MPC controller and evaluate it on a Unitree G1. The scope is deliberately bounded: full torque-level, long-horizon locomotion under the G1's wide stance is an orthogonal problem left to specialized gait schedulers; this work validates the interaction-dynamics *representation* — a robot-invariant predictor with constraint-aware realization — not a walking controller.

Table I summarizes the separation: Levels 1 and 3 predict future residual accelerations for the body and task ports, while Level 2 is a realizer with no horizon or future cost, solving only the current-sample feasibility problem that maps both requests into generalized torques.
...
**Table I.** The three levels of the architecture. Levels 1 and 3 are predictors (future residual accelerations for the body and task interaction ports); Level 2 is the instantaneous realizer, with no horizon or future cost.
...
**Fig. 1.** Floating-base whole-body manipulation as two normalized interaction-dynamics ports. The shared prediction object supplies the same constant exact-ZOH predictor to the body and task MPCs; their residual-acceleration commands are converted into centroidal and task wrenches before entering the whole-body interaction realizer. The realizer is not an MPC — it is an instantaneous inverse-dynamics QP that projects the requests onto feasible generalized torques for the G1/MuJoCo plant. Measured state, contacts, and wrenches feed the Kalman disturbance estimator; the green dashed path is the optional arm-reaction preview used by the coupled realization.
...
**Positioning.** We do not propose a locomotion controller or compete with the production gait stack on platforms such as the Unitree G1. The contribution is an *interaction-dynamics layer* on top of a mature balance/locomotion base: it predicts the two interaction ports, anticipates cross-port and external-contact effects, and emits a centroidal-wrench and CoM-residual correction that the base — or, when idle, the standing realizer of Section VI — turns into feasible joint commands. The representation and realization claims (H1–H5) are evaluated in fixed support; H6 demonstrates the layer on a moving base doing its own weight-shift; both stay in double support. The probes of Appendix A are compatibility and visualization studies, not evidence that this paper solves walking.
...
**Assumption 1 (exact feedforward normalization).** A controlled port has error coordinate $e$ and constrained dynamics $M_p(q,\rho)\ddot e + \mu_p(q,\dot q,\rho) = F_p^{\rm act} + F_p^{\rm ext}$, where the *interaction inertia* $M_p$ is invertible and well-conditioned on the operating set. The commanded actuation cancels the bias and desired-acceleration terms exactly, $F_p^{\rm act} = \mu_p + M_p(\ddot e_d + u) + \delta$, and the recovery map $u \mapsto F_p^{\rm act}$ is affine on each active-constraint cell.
...
Intuitively, the representation behaves like a stable software interface. The predictor is the interface — a fixed, robot-independent contract $\ddot e = u + d$ that upstream planning or learning can target without knowing the robot's mass, inertia, or contact state. The realizer is the implementation behind that interface: it absorbs the messy, configuration-dependent mechanics and reports back only how far short it fell, the residual $r$. Just as a caller of a well-designed API need not know the implementation, and different implementations can honor the same interface, a reference generator targeting the interaction-dynamics interface need not know the robot, and different whole-body realizers honor the same predictor (Fig. 2).

![Fig. 2. Prediction–realization interface.](figures/prediction_realization_concept.png)

**Fig. 2.** The prediction–realization interface of Definition 1 and Theorem 1. A reference generator (planner or learned policy) supplies interaction intent to the *predictor* — the robot-independent interface $\ddot e = u + d$ with constant $(A,B)$ — which emits a residual-acceleration command $u$. The *realizer* is the implementation behind the interface: an instantaneous whole-body QP that carries all robot mechanics ($M_p$, contacts, actuator limits), drives the robot, and returns the realization residual $r$. A Kalman observer estimates the interaction disturbance $d$ from the innovation and feeds it back. Prediction is future and robot-invariant; realization is present and robot-specific.

The remainder of the paper is a consequence of Definition 1 and Theorem 1. Sections IV–V construct the body and task ports as the two instances (Propositions 1–2 are Theorem 1 specialized to each port, with their explicit recoveries). Sections VI–VIII give the shared realizer and the estimator that closes the loop. Section IX states what the representation does and does not inherit once $r$ is present (Propositions 3–5). Section X validates the definition directly: that the predictor is invariant while everything robot-specific lives in recovery (H1), and that the resulting closed loop regulates, anticipates, respects constraints, and rides a moving base (H2–H6).
...
The evaluation uses a Menagerie-derived Unitree G1 model as the common plant. The comparisons are organized around hypotheses rather than around the software layers themselves. This is important because the paper's claim is representation-level: both interaction ports should share the same predictor, while the floating-base mechanics appear in recovery. H1–H5 are reported below and all hold on the standing torque-actuated G1 (H2 offset-free regulation is demonstrated on the full realizer, not only at the representation level), and H6 demonstrates the interaction layer on a moving base that commands its own weight-shift; only the sustained-walking form of the benchmark depends on the dynamic-gait realizer and remains future work. The six hypotheses form four groups: **representation** (H1 — the predictor is invariant), **regulation and anticipation** (H2–H3 — the closed loop is offset-free and previews what the realizer does not model), **detection and constraint realization** (H4–H5 — events are seen and constraints are honored, both in recovery), and **moving base** (H6 — the layer adds interaction on a base doing its own motion).

| Hypothesis | Comparison | Primary Evidence | Status |
|---|---|---|---|
| H1: normalized prediction is port-independent | conventional centroidal MPC vs. centroidal interaction MPC; same port across G1/H1/Talos | same $(A,B)$, command equivalence, $\Lambda_t$ variation ($3840\%$ over a sweep, $155\times$ across robots) | evaluated (Table II, Fig. 3) |
...
H1 is the empirical test of Theorem 1 and the centerpiece of the evaluation: its single message is that the predictor is invariant while everything robot-specific lives in recovery. Five measurements make this precise. First, the two *double-integrator* channels — the planar CoM double integrator of the body port (the CoM block of Proposition 1) and the hand double integrator of the task port — are the *same* constant exact-ZOH pair: measured against the closed-form (11), $\max\|A-A_\text{ZOH}\|=\max\|B-B_\text{ZOH}\|=0$ for both ports; only the dimension differs. Second, the body port's *angular-momentum* channel of Proposition 1 is checked in its own right: the normalized first-order integrator $\dot e_h=u_\theta+d_\theta$ has discrete pair $(A_\theta,B_\theta)=(I,T I)$, which equals the exact-ZOH first-order integrator to machine precision and is configuration-independent, while the centroidal rotational inertia $I_G(q)$ that the moment recovery must invert varies over the same arm sweep (eigenvalues $[2.4,6.9,8.5]\to[2.7,7.1,8.7]$ kg·m², $\approx$11%) — the momentum-channel analogue of the $\Lambda_t$ result. Third — the representation-versus-conventional ablation — the normalized centroidal MPC (decision = residual acceleration) and a conventional force-input centroidal MPC (decision = CoM force, matched weights $R_f=R/m^2$) are the same feedback map: over 2000 random states and disturbances the maximum command difference is $7.5\times10^{-6}$. The normalized representation therefore loses nothing relative to the traditional force-input formulation; it only relocates where the robot mechanics live. Fourth, over a 36-point right-arm kinematic sweep the contact-consistent task inertia $\Lambda_t$ (17) varies from $0.33$ to $13.1$ kg on its diagonal — up to $3840\%$ — while the predictor $(A_t,B_t)$ stays exactly constant (Fig. 4a, left). Fifth, and most directly for Theorem 1, the *same* task port is instantiated on three humanoids of very different scale — Unitree G1 ($34$ kg), Unitree H1 ($51$ kg), and PAL Talos ($94$ kg) — each with its two feet as the stance-contact set and its right-arm distal link as the end-effector. Across the three, the exact-ZOH predictor $(A_t,B_t)$ is *bit-identical* ($\max\|A-A_\text{ZOH}\|=\max\|B-B_\text{ZOH}\|=0$ for all), because it depends only on the sample time; yet the recovery quantities the realizer must invert span large factors — total mass $2.8\times$, centroidal rotational inertia across a comparable range, and the task apparent inertia $\Lambda_t$ from $1.2$ to $184$ kg, a **$155\times$** spread (Fig. 3). The predictor is therefore genuinely a property of the interaction-dynamics representation, not of the robot. All configuration, contact, and platform dependence lives in recovery, not in prediction, which is the H1 claim.
...
**Fig. 3.** H1 across three humanoids (Unitree G1, Unitree H1, PAL Talos). Each marker is a diagonal entry of the contact-consistent task apparent inertia $\Lambda_t$ at that robot's hand (log scale); the values span $1.2$–$184$ kg, a $155\times$ range across platforms. The exact-ZOH task predictor $(A_t,B_t)$ is identical for all three ($\max\|A-A_\text{ZOH}\|=\max\|B-B_\text{ZOH}\|=0$, boxed), confirming Theorem 1: the predictor is a property of the representation, not the robot.

### Results for H2 (Offset-Free Dual-Port Regulation)

H2 is tested in two settings. First, at the *representation level*, where the recovery is faithful so that $\ddot e=u+d$ holds exactly by construction (using the G1 mass for the body port and the contact-consistent $\Lambda_t$ for the task port), a sustained disturbance is applied and the steady-state error is compared with the disturbance observer disabled and enabled. The observer estimate converges to the true disturbance and the offset is removed. (At the representation level the *no-observer* number is the steady state of the open normalized model, not a physical excursion: because the lateral task apparent inertia is small, $\Lambda_t\!\approx\!0.4$ kg, an uncancelled 8 N residual maps to a large normalized offset — hence the meaningful quantity in that row is the offset *ratio*, whereas the physically scaled numbers are the full-realizer rows.) Second, the same test is run on the *full torque-actuated G1 realizer* of (22): the body-port residual acceleration is realized by driving the whole-body CoM linear acceleration to $\ddot c_d+u_c$ (equivalently, allocating the centroidal wrench $m(\ddot c_d-g)+m u_c$ across the contacts through the inverse-dynamics QP), and the task-port residual acceleration is realized as the end-effector acceleration $\ddot x_{t,d}+u_t$. With this faithful recovery in place, $\ddot e=u+d$ holds on the robot and the observer becomes consistent with the plant, so offset-free regulation carries over to the full realizer for both ports:
...
The three fixed-support hypotheses H3–H5 follow directly. All are realized by the torque-actuated whole-body QP (22). Its fixed-support push-rejection behavior, together with *locomotion-compatibility* probes that lie outside the interaction-dynamics claim of this paper — a root-assisted walking visualization and a Unitree open-source locomotion-stack demo — is collected in Appendix A.
...
**Table IV.** H3: peak and RMS lateral CoM excursion during a planned cross-port reaction, split vs. coupled prediction (Fig. 5a). The preview helps for the external wrench, which the whole-body QP does not model, but *not* for the internal arm momentum: the QP's CoM objective already drives the full $J_{\rm com}\ddot q$ across arm and legs, so an equal-magnitude arm swing produces a $4\times$ smaller uncompensated transient (9.2 vs 37.7 mm) with no preview, and adding the external-style preview only perturbs a CoM command the shared Jacobian is already satisfying. Preview therefore belongs to what the realizer does not model — external contact loads — while internal-momentum coupling is handled natively by the unified realizer.
