...
Nor is the framework a complete locomotion stabilizer. It assumes a shared locomotion stack that supplies the nominal body, foot, and contact references, and it modifies none of them — not foot placement, gait timing, or the contact schedule. Diagnostic analysis confirms that this shared stack tracks its own lateral divergent-component-motion (DCM) reference accurately (about $4$ mm RMS on flat ground); consequently the long-horizon walking failures observed outside the evaluation window cannot be attributed to persistent DCM tracking error. Preliminary experiments with a shared DCM-based foot-placement stabilizer indicate that extending locomotion robustness requires coordinated changes to gait-reference generation, foot-placement policy, and whole-body realization together, rather than interaction prediction alone; these locomotion components are treated as shared infrastructure and left outside the scope of this work. Because the shared stack's long-horizon behavior is common to every controller, the primary comparisons are conducted over a fixed evaluation window in which it remains repeatable.
