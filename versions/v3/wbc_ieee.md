...
**Definition 1 (interaction-dynamics representation).** An *interaction-dynamics representation* of a controlled port is a normalized prediction model $\ddot e = u + d$ whose state-transition pair $(A,B)$ is independent of the robot's mechanics — mass, centroidal and task inertia, contact geometry, friction, and actuator limits — together with an instantaneous *realization* (recovery) map that carries all of that robot- and configuration-specific dependence and projects the requested port command onto the feasible whole-body dynamics, leaving a *realization residual* $r$.

The remainder of the paper is a consequence of Definition 1. Sections IV–V construct the body and task ports as two instances of the representation, with their recoveries (Propositions 1–2). Sections VI–VIII give the shared realizer and the estimator that closes the loop. Section IX states what the representation does and does not inherit once $r$ is present (Propositions 3–5). Section X validates the definition directly: that the predictor is invariant while everything robot-specific lives in recovery (H1), and that the resulting closed loop regulates, anticipates, respects constraints, and rides a moving base (H2–H6).
