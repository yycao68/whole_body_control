...
*Abstract*—Floating-base robots—humanoids, quadrupeds, and legged manipulators—must simultaneously balance on compliant terrain, respect rigid ground contact constraints, and execute safe, dexterous interaction with humans or objects. Existing whole-body control (WBC) frameworks either allocate the full joint space to locomotion, leaving no principled mechanism for compliant manipulation, or rely on fixed-gain impedance feedback that accumulates steady-state error under sustained physical human–robot interaction (pHRI) forces. This paper develops the *interaction layer* of a floating-base WBC hierarchy, extending the fixed-base two-layer Impedance MPC to this setting: a receding-horizon quadratic program (QP) that occupies the residual null space of the balance tasks and predicts and rejects pHRI disturbances at the arm end-effector using a Kalman-augmented disturbance state. The surrounding balance hierarchy—a centroidal MPC over a single rigid-body dynamics (SRBD) abstraction and a priority-driven contact-consistent WBC—is the architecture into which this layer is designed to plug; the simulations run the interaction layer above a stabilizing balance controller. The key structural result is that a contact-consistent feedback linearization reduces the arm end-effector plant to a linear double integrator with a *constant* state matrix within each contact mode, enabling offline precomputation of the QP cost inverse and operation at ≥1 kHz. A covariance-inflation protocol handles contact-mode transitions without resetting the disturbance estimate, giving zero steady-state tracking error under bounded constant pHRI loads at fixed contact mode. We prove an Impedance Equivalence Theorem establishing that the infinite-horizon, zero-input limit of the proposed MPC recovers a classical task-space impedance law whose closed-loop effective mass $M_{d,\text{eff}}$, damping $D_\text{eff}$, and stiffness $K_\text{eff}$ all adapt to arm posture and contact configuration through the contact-consistent inertia $\Lambda_\text{arm}(q)$, without requiring online re-optimization of impedance parameters. Stability is analyzed under fixed-mode contact with explicit transient bounds across contact events. In simulation on a 17-DOF biped and a Unitree G1 model, the primary benefit of the contact-consistent formulation is **stability** under dynamic contact loading: a fixed-base impedance MPC of comparable gain diverges under periodic contact shocks, whereas the contact-consistent layer stays stable and sub-millimetre. In static stance the two are comparable, and the Kalman augmentation provides a modest offset-free refinement.
...
This scenario validates the covariance-inflation protocol. At each contact-mode switch the Kalman input matrix $B_d^{(m)}$ jumps discontinuously; without inflation (D6) the stale covariance briefly biases $\hat{d}$, so the peak transition error (4.67 mm) is in fact *worse* than the no-Kalman controller (D5, 4.50 mm). Inflating the covariance at the switch (D7, $\alpha=4$) widens the filter, accelerates re-estimation of $\hat{d}$ under the new $B_d$, and recovers the lowest peak (4.42 mm). The effect is modest in magnitude but confirms the protocol's mechanism on a genuine contact-set change; a full single↔double-support transition, which requires the Level-1 balance controller, remains future work.

---

## XII. Conclusion

This paper developed the *interaction layer* of a floating-base whole-body controller: a null-space Impedance MPC that rejects pHRI disturbances at the arm end-effector with zero steady-state error at fixed contact mode, within the residual null space of the balance tasks. The surrounding balance hierarchy—a centroidal MPC and a priority-driven contact-consistent WBC—is the architecture into which the layer is designed to plug; the simulations here run it above a stabilizing balance controller, and closing the loop with the full balance stack is a primary direction for future work. The central technical contribution is the proof that a contact-consistent feedback linearization reduces the arm end-effector plant to a linear double integrator with a constant discrete state matrix within each contact mode, enabling offline precomputation of the QP cost inverse and ≥1 kHz operation. The contact-mode-indexed input-matrix library and a Kalman covariance-inflation protocol carry these properties across contact transitions. The inflation protocol is validated on the bracing-hand support transition of Scenario E, where the active contact set genuinely switches while both feet stay planted: inflation recovers the lowest peak transition error, and without it the Kalman is briefly worse than the no-Kalman controller at each switch. A full single↔double-support transition — which requires the Level-1 balance controller outside the interaction layer studied here — remains future work. The Impedance Equivalence Theorem establishes formal equivalence with classical operational-space impedance control in the infinite-horizon limit, facilitating impedance parameter design.
...
In simulation, the decisive benefit of the contact-consistent formulation is **stability**: under dynamic contact shocks a fixed-base impedance MPC of comparable gain diverges, while the contact-consistent layer stays sub-millimetre, because $\Lambda_\text{arm}$ correctly reflects the higher contact-coupled apparent inertia and hence a well-conditioned predictive gain. In static stance the contact-consistent and fixed-base controllers are comparable, and the Kalman augmentation yields a modest offset-free refinement — the large steady-state-error reductions attainable at a fixed configuration are gain-dominated rather than a unique property of the disturbance observer.
