...
\item \emph{评估。} 在 Unitree G1 上开展的一项配对人形机器人研究，包含专用控制器、仅质心和 oracle 消融实验、统计学意义上的推力与持续力测试、传感鲁棒性扫描，以及置信门引入前后的捕获放大特性。
\end{enumerate}
...
\textbf{V1（估计器正确性）。} 我们在关闭交互层的情况下，记录了真实施加力与 $\hat F_{\rm ext}$ 在名义行走、指令转向、对角（侧向）行走、30~mm 台阶下降、120~N/0.15~s 冲击、以及 12~N/3~s 持续力等场景下的对比（图~\ref{fig:estimator}）；盲台阶上升未列入表~\ref{tab:estimator}，因为机器人在该场景中会绊倒，它改在演示部分（第~\ref{sec:demo}节）中出现。V1 脚本确实记录了该轨迹：其统计量仅取自跌倒前的样本（截至检测到跌倒时刻 $2.55$~s 之前 $0.2$~s），在该区间内估计器保持 $0.15$~N 的 RMSE 且无误报，因此该绊倒是运动失败而非被误判的外力。虚警率定义为在无外力样本中 $|\hat F_{\rm ext}|>F_{\rm cap}$ 所占的比例。
...
\begin{table}[t]
\caption{Oracle 消融实验（每列采用 20 个配对种子）。}
\label{tab:ablation}
\centering
\begin{tabular}{lrr}
\hline
...
保持专用 & 20 & 10\\
仅质心统一控制器 & 16 & 719\\
\textbf{力门控} & \textbf{12} & \textbf{14}\\
oracle & 11 & 10\\
\hline
\end{tabular}
\end{table}

专用响应具有明确的适用模式：纯捕获会\emph{放大}持续力（1080~mm，而策略为 462~mm），保持控制则没有改善瞬态跌倒次数。在所测试的控制器族与扰动协议下，仅质心仲裁得到 16 次跌倒和 719~mm 漂移，而基于外力持续性的仲裁得到 12 次跌倒和 14~mm 漂移（表~\ref{tab:ablation}）。该结果说明外力持续性对本文所评估仲裁方式的价值，但不表示它对每一种可能的分类器在数学上都是必需的。力门控控制器在两列中都接近 oracle。

\textbf{V3（泛化能力，留出集测试）。} 在所有阈值与增益均保持冻结的前提下，我们评估了从未用于调参的情形：留出的冲击\emph{持续时间}（0.10/0.25/0.40~s，相对于设计时使用的 0.15~s）、网格之外的持续力幅值（6/10/14~N）、斜坡上升力，以及间歇性外力。在留出的 0.25~s 持续时间下，控制器与捕获专用控制器表现相当；网格之外的持续力幅值沿已标定趋势变化；对于 1~s 斜坡外力，其漂移小于裸策略。\textbf{分类边界：} 一个快速间歇性外力（0.5~s 开/关）介于重复推力与持续力之间，持续性混合不能为该情形选择稳定的工作模式；第~\ref{sec:limits}节将其作为运行边界。
...
\textbf{捕获放大机制。} 仅凭平衡类指标无法揭示侧向位置响应：当过程噪声超出死区时，未加门控的捕获路径会放大已介入但无法归类为外力的漂移，在 4~N 噪声下产生 1539~mm 的无外力漂移，但并不导致跌倒。捕获朝未分类漂移方向迈步，由此闭合正反馈路径。第一级置信门（第~\ref{sec:gate}节）在外力估计提供交互证据前关闭该路径。加入门控后，无外力漂移降至约 $95$~mm（策略自身的本底水平），同时瞬态与持续结果得以保持或改善（表~\ref{tab:ablation}–\ref{tab:stats}）。\footnote{一段补充视频在仿真 G1 上并排展示了该机制：在相同的过程噪声、且没有真实外力的条件下，强制打开门控会重现修复前未加门控控制律的漂移失控（在所渲染的种子上漂移超过 1000~mm），而已发布的门控则将漂移保持在数十毫米量级，与噪声本底相当。}在线的、按噪声缩放的阈值（式~\eqref{eq:fcap}）无需手工调参，就能自动恢复出适合特定噪声水平的取值：
...
在冻结策略之上，该层于行走过程中原位（in-situ）运行。在侧向单支撑推力下，它扩展了恢复包络（图~\ref{fig:envelope}，左）；在侧向持续力下，它将质心保持在名义路径附近，而此时策略本身会出现漂移（图~\ref{fig:envelope}，右）。在地形方面，策略本身已经能够处理的\textbf{台阶下降}不受该层影响，而超过几厘米的盲目\textbf{台阶上升}则会导致所有控制器都出现脚趾绊碰式的\emph{绊倒}。这一区分明确了各层职责：交互控制响应已经测得的物理效应，地形几何则必须由感知与规划提供。第~\ref{sec:discussion}节进一步说明这一系统边界。
...
\textbf{交互分类。} 周期接近检测时间尺度的间歇性施力无法形成稳定的持续性分类（V3）。置信门处理低于阈值的噪声，而间歇性情形需要面向重复施力设计的分类器或控制模式。
\textbf{实现接口。} 用本文评估的全身 QP 替换学习策略的低层回路会产生不稳定的行走响应（第~\ref{sec:realize}节），因此本文通过策略指令接口注入交互修正。带有 QP 原生接口的模型步行器属于另一种实现情形。
\textbf{系统边界。} 地形几何与扰动预判由感知和规划提供；交互层在物理效应可观测后响应，并将自身权限边界向上传递。
现有证据覆盖一个仿真人形机器人和\textbf{一个步行策略}。接口在构造上与策略实现解耦，而经验层面的迁移仍需在其他策略、任务坐标、长距离行走、可变形地面和同时操作任务上验证。

\section{结论}
\label{sec:conclusion}

我们提出了交互动力学（Interaction Dynamics）：一个掌管腿式机器人与世界之间物理交互的快速、短时间尺度的层。它将所有外部交互都表示为一个构型不变的固定模型之上的残差，从质心动量残差中估计外力，并借助一个在线的、按噪声缩放的置信门，以及一个基于持续性、在符号相反的捕获与保持之间进行混合的机制来对其进行推理。该层通过指令接口叠加在一个\emph{冻结}的强化学习策略之上，能够减少推力导致的跌倒（在所测试的推力幅值下没有出现任何配对意义上的性能退化），并将持续力抑制到接近专用控制器的精度，同时名义行走性能不受影响；一个六阶段的验证支持了上述结论，并刻画了置信门所处理的捕获放大机制。我们提出交互动力学作为感知驱动的规划与全身实现之间的一个候选\emph{快速物理交互层}——它是一个基于模型的 Physical AI 层级结构中的一级；端到端规划器耦合属于独立的验证阶段。
