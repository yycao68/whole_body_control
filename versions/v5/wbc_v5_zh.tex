...
\textbf{捕获放大机制。} 仅凭平衡类指标无法揭示侧向位置响应：当过程噪声超出死区时，未加门控的捕获路径会放大已介入但无法归类为外力的漂移，在 4~N 噪声下产生 1539~mm 的无外力漂移，但并不导致跌倒。捕获朝未分类漂移方向迈步，由此闭合正反馈路径。第一级置信门（第~\ref{sec:gate}节）在外力估计提供交互证据前关闭该路径。加入门控后，无外力漂移降至约 $95$~mm（策略自身的本底水平），同时瞬态与持续结果得以保持或改善（表~\ref{tab:ablation}–\ref{tab:stats}）。在线的、按噪声缩放的阈值（式~\eqref{eq:fcap}）无需手工调参，就能自动恢复出适合特定噪声水平的取值：
