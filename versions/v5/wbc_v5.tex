...
\item \emph{Evaluation.} A paired humanoid study on a Unitree G1 with specialist, CoM-only, and oracle ablations, statistical push and sustained-force tests, sensing-robustness sweeps, and characterization of capture amplification before and after confidence gating.
\end{enumerate}
...
\textbf{V1 (estimator correctness).} We logged the true applied force against $\hat F_{\rm ext}$ (interaction layer off) across nominal walking, commanded turning, diagonal (lateral) walking, a 30~mm step-down, a 120~N/0.15~s impulse, and a 12~N/3~s sustained force (Fig.~\ref{fig:estimator}); the blind step-up is excluded from Table~\ref{tab:estimator} because the robot trips on it, and appears instead in the demonstration (Section~\ref{sec:demo}). The V1 script does log that trace: it is scored only on pre-fall samples (up to $0.2$~s before the detected fall at $2.55$~s), over which the estimator holds $0.15$~N RMSE with no false positive, so the trip is a locomotion failure rather than a mislabelled force. False positive is the fraction of no-force samples with $|\hat F_{\rm ext}|>F_{\rm cap}$.
...
\begin{table}[t]
\caption{Oracle ablation (20 paired seeds per column).}
\label{tab:ablation}
\centering
\begin{tabular}{lrr}
\hline
...
hold-specialist & 20 & 10\\
CoM-only unified & 16 & 719\\
\textbf{wrench-gated} & \textbf{12} & \textbf{14}\\
oracle & 11 & 10\\
\hline
\end{tabular}
\end{table}

The specialist responses are regime-specific: pure capture \emph{amplifies} a sustained force (1080~mm versus 462~mm for the policy), while hold does not improve the transient fall count. In the tested controller family and disturbance protocol, CoM-only arbitration gives 16 falls and 719~mm drift, whereas wrench persistence gives 12 falls and 14~mm drift (Table~\ref{tab:ablation}). This establishes the value of force persistence for the evaluated arbitration, not its mathematical necessity for every possible classifier. The wrench-gated controller operates near the oracle in both columns.

\textbf{V3 (generalization, held-out).} With all thresholds and gains frozen, we evaluated cases never used for tuning: held-out impulse \emph{durations} (0.10/0.25/0.40~s vs.\ the 0.15~s used in design), off-grid sustained magnitudes (6/10/14~N), ramped forces, and intermittent forces. At the held-out 0.25~s duration the controller matches the capture specialist; off-grid sustained magnitudes follow the calibrated trend; and a 1~s ramp produces less drift than the bare policy. \textbf{Classification boundary:} a rapidly intermittent force (0.5~s on/off) remains ambiguous between repeated pushes and a sustained force. The persistence blend does not select a stabilizing regime for that case, which defines an operating boundary in Section~\ref{sec:limits}.
...
The transient benefit is strongly significant, and no paired trial was worsened at the three evaluated push magnitudes (Table~\ref{tab:stats}); ``never worse'' is stated for these conditions (cf.\ the intermittent-force failure in V3). On sustained force at 8~N the controller's drift lies within the hold specialist's spread (13 [8--17] vs.\ 12 [7--19]~mm); a paired equivalence test would establish formal equivalence. The causal-detection cost is a modest $\sim 1.7\times$ at 12~N. The envelope (Fig.~\ref{fig:envelope}) shows the controller holding 0\% single-support falls through 280~N while the policy is already 60--87\%.
...
\textbf{Capture-amplification mechanism.} Balance metrics alone do not reveal a lateral-position response: under process noise exceeding the deadband, an ungated capture path amplifies drift that it engages on but cannot classify as a force, producing 1539~mm no-force drift at 4~N noise without a fall. Capture steps toward the unclassified drift and closes a positive-feedback path. The Level-1 confidence gate (Section~\ref{sec:gate}) disables that path until the wrench estimate provides evidence of an interaction. With the gate, no-force drift is $\sim 95$~mm (the policy's own floor), while the transient and sustained results are preserved or improved (Tables~\ref{tab:ablation}--\ref{tab:stats}).\footnote{A supplementary video renders this mechanism side by side on the simulated G1: with identical process noise and no real external force, forcing the gate open reproduces the pre-fix ungated law's runaway (over 1000~mm of drift on the rendered seed), while the shipped gate stays within tens of millimeters, matching the noise floor.} The online noise-scaled threshold \eqref{eq:fcap} recovers the value appropriate to a fixed noise level without hand-tuning:
...
On the frozen policy, the layer runs in-situ during walking. Under lateral single-support pushes it extends the recovery envelope (Fig.~\ref{fig:envelope}, left) and, under sustained lateral force, it holds the CoM near the nominal path where the policy drifts (Fig.~\ref{fig:envelope}, right). On terrain, a \textbf{step-down} the policy already handles is unaffected by the layer, while a blind \textbf{step-up} beyond a few centimeters causes a toe-stub \emph{trip} for every controller. This separates the roles of the layers: interaction control responds to measured physical effects, whereas terrain geometry must enter through perception and planning. Section~\ref{sec:discussion} states this system boundary explicitly.
...
The proposed layer complements perception and motion planning: it operates on a shorter time scale and solves a different problem, serving as the \emph{bridge} between reactive physical control and deliberative model-based planning.

\textbf{Perception and interaction are complementary.} Perception and a world model estimate \emph{what the world is}; motion planning decides \emph{what to do next}; Interaction Dynamics governs \emph{how the robot physically interacts in the meantime}, stabilizing on the estimated interaction while the slower loops update the plan. The blind step-up (Section~\ref{sec:demo}) makes this concrete: terrain perception is what lets the planner raise or shift the foothold, while the layer stabilizes through the planning latency. Interaction control and perception occupy different, complementary rungs of the stack; the layer stabilizes through the planning latency rather than compensating for perception delay.

\textbf{The layer buys time.} The same holds for external forces. A push is met immediately, because the response needs only $\hat F_{\rm ext}$. If the disturbance grows beyond the layer's authority, a higher layer can widen the stance, slow down, change direction, abandon the task, or enter a safe mode. The observed recovery margin suggests the layer could provide additional time for a slower planning response (the delayed planner reaction is a separate test):
\begin{center}
...
A higher layer could consume the estimated wrench, its regime, and falling rate $\dot\theta_{\rm fall}$. We treat $\dot\theta_{\rm fall}$ as an instability cue; a calibrated authority margin would combine command saturation, foot-placement reach, and support-polygon limits, with $\dot\theta_{\rm fall}$ correlated against the recovery boundary. This upward path is a proposed interface: the layer exposes its interaction state, and closing the loop to a planner is a separate study.

\textbf{Where this places the contribution.} In a model-based Physical AI stack---perception $\rightarrow$ planner $\rightarrow$ interaction dynamics $\rightarrow$ whole-body realization $\rightarrow$ robot---the contribution is \emph{one rung}: the fast physical-interaction interface between planning updates. ``Physical AI'' here is the framing context; the paper defines a single rung with a clear responsibility (estimate, reason about regime, stabilize) and a clear interface above and below.

\section{Operating Envelope and Validation Scope}
\label{sec:limits}

\textbf{Interaction classification.} Intermittent forcing at a period near the
detection time scale does not produce a stable persistence classification
(V3). The confidence gate addresses sub-threshold noise, while the intermittent
case requires a classifier or control mode designed for repeated forcing.
\textbf{Realization interface.} Replacing the learned policy's low-level loop
with the evaluated whole-body QP produced an unstable walking response
(Section~\ref{sec:realize}), so this study injects the correction through the
policy's command interface. A model-based walker with a QP-native interface is
a distinct realization case.
\textbf{System boundary.} Terrain geometry and disturbance anticipation enter
through perception and planning; the interaction layer responds after physical
effects become observable and exposes its authority boundary upward.
The evidence covers one simulated humanoid and \textbf{one locomotion policy}.
The interface is policy-agnostic in construction, while empirical transfer
requires evaluation on additional policies, task coordinates, long-distance
walking, deformable ground, and simultaneous manipulation.
...
and characterizes the capture-amplification mechanism addressed by the confidence gate. We propose Interaction Dynamics as a
candidate \emph{fast physical-interaction layer} between perception-driven planning and whole-body 
realization---one rung of a model-based Physical AI hierarchy. End-to-end planner coupling is a separate validation stage.
