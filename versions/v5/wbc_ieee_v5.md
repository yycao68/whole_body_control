...
4. *Evaluation.* A paired humanoid study on a Unitree G1 with specialist, CoM-only, and oracle ablations, statistical push and sustained-force tests, sensing-robustness sweeps, and a capture-amplification failure mode resolved by the confidence gate.
...
**Capture-amplification failure mode.** Balance metrics alone hide a failure visible only in lateral position: under process noise exceeding the deadband, an ungated capture path amplifies any drift it engages on but cannot classify as a force, driving a runaway lateral excursion (no-force drift 1539 mm at 4 N noise) that never causes a fall. This is a control-logic problem, not a tuning one — capture steps toward an unclassified drift, closing a positive feedback loop — and it is what the Level-1 confidence gate (Sec. V-A) is designed to prevent, by enabling capture only on confident evidence of a real interaction. With the gate, the no-force drift falls to $\sim 95$ mm (the policy's own floor), while the transient and sustained benefits are preserved or improved (Tables II–III). The online noise-scaled threshold (5) recovers the value appropriate to a fixed noise level without hand-tuning:
...
[1] Y. Cao and J. Tang, "Compact Offset-Free Interaction-Error MPC with Applied-Torque Constraints for Physical Human–Robot Interaction," 2026, arXiv:2606.08281.
