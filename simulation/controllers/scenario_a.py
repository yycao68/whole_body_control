...
def run_controller(name, cfg):
    ...
    # ── WBC setup ─────────────────────────────────────────────────────
    # Contact-consistent projection is applied on the WBC/MPC path (D4-D7);
    # the fixed-base baseline (D3, use_cc=False) and the pure PD/PI baselines
    # (D1/D2) run without it.
    use_cc   = cfg.get('use_contact_consist', True)
    use_proj = bool(cfg.get('use_wbc') or cfg.get('use_mpc')) and use_cc
    ...
    for step in range(N_RUN):
        ...
        if mpc is not None:
            ...
            Jc  = get_contact_jacobian(model, data, foot_site_ids, [True, True])
            if use_cc:
                Mbar = get_contact_consistent_inverse(M, Jc)          # M̄
            else:
        ...
        if mpc is not None:
            # Refresh the contact-mode B_d for the current Lambda_arm(q) and
            # keep the Kalman's model in sync (offset-free needs matching B_d).
            mode = mpc.get_or_update_mode('ds', Lam_arm)
            d_hat = None
