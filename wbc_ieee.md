# Interaction Dynamics for Floating-Base Robots: A Contact-Consistent, Constant-Model Predictive Interaction Layer for Safe Physical Human–Robot Interaction
...
*Abstract*—Safe physical interaction on floating-base robots—humanoids, quadrupeds, and legged manipulators—is fundamentally a problem of *interaction dynamics*: the closed-loop relation between the arm end-effector force and motion, coupled through rigid ground contact to the whole-body balance task. Most whole-body control (WBC) frameworks remain *motion-centric*—they optimize robot motion subject to contact constraints but do not treat interaction dynamics as the object of prediction and optimization—and fall back on fixed-gain impedance feedback that accumulates steady-state error under sustained physical human–robot interaction (pHRI) forces. This paper instead formulates safe whole-body interaction as a *model-based interaction-dynamics* problem and develops the corresponding *interaction layer* of a floating-base WBC hierarchy. It extends the fixed-base interaction-dynamics reduction of our prior Impedance MPC [23] to the floating-base, contact-switching setting: a receding-horizon quadratic program (QP) occupies the residual null space of the balance tasks and predicts and rejects pHRI disturbances at the arm end-effector using a Kalman-augmented disturbance state. The key structural result is that a contact-consistent feedback linearization *normalizes* the floating-base interaction dynamics into a linear double integrator with a *constant* state matrix within each contact mode—identical in form to the fixed-base reduction, with contact switching not breaking this structure but only reindexing the input matrix $B_d^{(m)}$—enabling offline precomputation of the QP cost inverse and operation at ≥1 kHz. A covariance-inflation protocol carries the disturbance estimate across contact-mode transitions, giving zero steady-state tracking error under bounded constant pHRI loads at fixed contact mode. We prove an Impedance Equivalence Theorem establishing that the infinite-horizon, zero-input limit of the proposed MPC recovers a classical task-space impedance law whose closed-loop effective mass $M_{d,\text{eff}}$, damping $D_\text{eff}$, and stiffness $K_\text{eff}$ all adapt to arm posture and contact configuration through the contact-consistent inertia $\Lambda_\text{arm}(q)$, without requiring online re-optimization of impedance parameters. Stability is analyzed under fixed-mode contact with explicit transient bounds across contact events. Our central result is one of **stability under dynamic contact loading**: in simulation on a 17-DOF biped and a Unitree G1 model, a fixed-base impedance MPC of comparable gain *diverges* under periodic contact shocks (RMS 372 mm), whereas the contact-consistent interaction layer stays stable and sub-millimetre (0.82 mm) — a stability margin unattainable with a free-space inertia model, because the task inertia $\Lambda_\text{arm}$ correctly captures the contact-coupled apparent inertia at the hand. The surrounding balance hierarchy—a centroidal MPC over a single rigid-body dynamics (SRBD) abstraction and a priority-driven contact-consistent WBC—is the architecture into which this layer is designed to plug; the simulations run the interaction layer above a stabilizing balance controller.

*Index Terms*—Interaction dynamics, model-based physical interaction, whole-body control, model predictive control, impedance control, floating-base robots, physical human–robot interaction, operational space formulation, Kalman filter, contact-consistent dynamics, legged manipulation.
...
We argue that safe whole-body interaction should instead be posed as a *model-based interaction-dynamics* problem: rather than optimizing motion alone, the controller should predict how interaction forces and errors evolve under changing contacts, actuator limits, and floating-base dynamics. The technical gap is thus the absence of a *predictive*, *disturbance-rejecting*, compliance-controlled interaction layer that natively integrates with an existing balance stack without degrading balance performance. This paper closes that gap by adopting the fixed-base interaction-dynamics reduction of our prior Impedance MPC [23]—which normalizes a fixed-base manipulator's interaction dynamics into a constant-model predictive plant—and extending it to floating-base, contact-switching robots, with the following contributions:

1. **Configuration-normalized floating-base interaction dynamics** (Section IV)—the central structural result. We show that the floating-base arm end-effector interaction dynamics, after a contact-consistent, priority-driven feedforward cancellation, reduce *exactly* to a linear double integrator with a *constant* discrete state matrix $A_d$ within each contact mode—the same constant-model predictive plant established for fixed-base manipulators [23]. The sole modification from the fixed-base case is the substitution of the contact-consistent mass inverse $\bar{M}^{-1}$ for the free-space inverse $M^{-1}$ in the operational-space inertia $\Lambda_\text{arm}$, with all configuration and contact dependence confined to the input matrix. Crucially, contact switching does not break this structure: it only reindexes the input matrix $B_d^{(m)}$ and the active contact mode, so the floating-base interaction problem *inherits* the constant-model predictive structure of the fixed-base case. The subsequent contributions are the machinery that turns this model into a real-time controller.
...
3. **Kalman disturbance isolation** (Section V-E): the augmented Kalman state estimates external pHRI forces, unmodeled leg-momentum variations, and SRBD approximation errors, and propagates the estimate through the prediction horizon, yielding offset-free tracking at a fixed contact mode (Theorem 2).
...
7. **Stability under dynamic contact loading** (Section X): we show empirically that the contact-consistent inertia is what makes predictive interaction control *stable* under time-varying contact. A fixed-base impedance MPC of comparable gain—indistinguishable from ours in static stance—diverges under periodic contact shocks (RMS 372 mm), whereas the contact-consistent layer remains stable and sub-millimetre (0.82 mm), because $\Lambda_\text{arm}$ reflects the true contact-coupled apparent inertia and hence a well-conditioned predictive gain.

The remainder of this paper is organized as follows. Section II surveys related work. Section III develops the floating-base dynamics and contact constraint formulation. Section IV derives the contact-consistent operational-space plant. Section V presents the three-level Impedance MPC architecture. Section VI describes contact-transition handling. Section VII states and proves the Impedance Equivalence Theorem. Section VIII analyzes stability. Section IX gives the complete joint torque equation and an architectural comparison with prior work. Section X reports simulation benchmarks on two robot platforms. Section XI concludes.
...
The present paper is a direct structural extension of the authors' prior work on saturated and predictive control. Anti-windup designs for output tracking under actuator saturation and constant disturbances [14], and the associated domain-of-attraction analysis [15], established that an integrating disturbance channel achieves zero steady-state tracking error for fixed-base saturated linear systems—the foundational insight carried forward here to the floating-base, contact-switching setting via the Kalman augmented state. The min–max MPC formulation for LPV systems [16] introduced parameter-varying input matrices with input constraints, the direct precursor to the contact-mode-indexed $B_d^{(m)}$ library of the present work. Building on these fixed-base results, Cao, Cheng, and Li [2] introduced a passive MPC framework for pHRI on fixed-base manipulators in which the outer MPC optimizes impedance parameters $\{M_d, D_d\}$ over a receding horizon; passivity is enforced via a virtual energy tank, building on the passivity arguments of [20]. Because impedance parameters enter nonlinearly into the prediction matrices, iterative solvers are required and update rates are limited to 10–30 Hz. The two-layer Impedance MPC for pHRI [23] resolved this rate limitation on fixed-base manipulators: an analytical feedforward cancels gravity, Coriolis, and task-space inertia, reducing the residual plant to a double integrator with a *constant* state-transition matrix, while the MPC decision variables become corrective *forces* $F_\text{mpc}$ and a Kalman-augmented disturbance state provides a formal zero-steady-state-error guarantee — sub-0.05 mm under a sustained 15 N force on a 7-DOF manipulator, versus 44.8 mm for classical impedance control. That framework is the direct base of the present paper, which extends it from fixed-base manipulators to floating-base humanoids: the constant-$A_d$ structure is preserved by substituting the contact-consistent mass inverse $\bar{M}^{-1}$ into the feedforward linearization, the Kalman state additionally absorbs leg-momentum variations and SRBD approximation errors, and the contact-mode-indexed structure of [16] handles stance-phase transitions without sacrificing the precomputed cost matrix structure. Cast in the terms of this paper, [23] normalized a fixed-base manipulator's *interaction dynamics* into a constant-model predictive plant; the present work shows this interaction-dynamics reduction is not an artifact of the fixed base — it survives rigid contact and, indexed by contact mode, contact switching — making floating-base robots the second setting in which safe interaction reduces to the same predictive interaction model.
...
This residual steady-state error under sustained pHRI is the fundamental limitation of the OS PD law and the primary motivation for replacing the arm task slot with Impedance MPC.
...
*Proof:* The continuous residual error dynamics (19) have state matrix $A_c = \left[\begin{smallmatrix}0 & I_3 \\ 0 & 0\end{smallmatrix}\right]$, which is nilpotent ($A_c^2 = 0$). The matrix exponential therefore terminates at first order, $A_d = e^{A_c\Delta t} = I + A_c\Delta t$, which is *exact* and independent of configuration—the double-integrator structure of (20). Evaluating $\Lambda_\text{arm}(q_k)$ at the current sample freezes $B_d(k) = [0;\, -\Lambda_\text{arm}^{-1}(q_k)\Delta t]$ over the horizon, so $(A_d, B_d(k))$ is LTI within the receding window; the configuration drift between Level-2 updates enters only $B_d$ and is first-order small. $\square$
...
with process noise $w \sim \mathcal{N}(0, Q_w)$ and measurement noise $v \sim \mathcal{N}(0, R_v)$ on arm end-effector position. Unlike the strict random-walk model of [23], in which process noise drives only the disturbance state, $Q_w$ here retains a small block on $x_e$ — two orders of magnitude below the $\hat{d}$ block (Section X) — to regularize the filter against the frozen-matrix and SRBD residuals of the floating-base setting; the offset-free property is unaffected, as it follows from the integrating structure of $A_\text{aug}$, not from the noise partition. The Kalman gain $K_f$ is computed offline from the steady-state discrete algebraic Riccati equation. The integrating structure of $A_\text{aug}$ guarantees $\hat{d}(k) \to d$ for any bounded constant disturbance, independent of its physical origin. This is the predictive-control analogue of the anti-windup result of [14], which shows that augmenting an integrating channel achieves zero steady-state error under actuator limits and persistent constant disturbances: here the Kalman state $\hat{d}$ takes the role of that integrating channel and feeds it forward through the MPC horizon rather than feeding it back as a fixed integral gain.
...
**Theorem 1** (Whole-Body Impedance Equivalence). *Under rigid contacts, fixed contact mode, and no disturbance, the unconstrained infinite-horizon Impedance MPC (Level 3, $N \to \infty$) renders, for the resulting impedance gains $(K_d, D_d)$, the classical task-space impedance law:*
...
*Proof.* In the infinite-horizon unconstrained limit with $\hat{d}=0$, the QP reduces to the static discrete-LQR (Riccati) feedback $F_\text{mpc} = K_\infty x_e$. Factoring the Riccati gain through the task inertia, write $F_\text{mpc} = \Lambda_\text{arm}(q)(K_d e + D_d \dot{e})$; this defines the acceleration-level gains $(K_d, D_d)$. Substituting into (19) with $d = \Lambda_\text{arm}^{-1}F_h$ gives
$$\ddot{e} = -(K_d e + D_d \dot{e}) + \Lambda_\text{arm}^{-1}F_h,$$ 

and premultiplying by $\Lambda_\text{arm}$ yields (25). Hence the effective mass, damping, and stiffness are $M_{d,\text{eff}} = \Lambda_\text{arm}(q)$, $D_\text{eff} = \Lambda_\text{arm}D_d$, and $K_\text{eff} = \Lambda_\text{arm}K_d$. The gains $(K_d, D_d)$ are the Riccati image of the QP weights $(Q, R)$, *not* the weights themselves, and the cheap-control limit $R \to 0$ does not equal $(K_d, D_d)$. The contact-consistent $\bar{M}^{-1}$ in $\Lambda_\text{arm}$ means the effective mass adapts to both the arm joint configuration and the active contact footprint. $\square$

Theorem 1 establishes that Impedance MPC generalizes classical impedance control: the finite-horizon, constrained QP adds predictive disturbance rejection, constraint enforcement, and contact-mode adaptation while reducing to the classical law in the limit. It also provides a physical interpretation for the cost weights $K_d$, $D_d$ as the desired impedance stiffness and damping. In the simulations of Section X, the Level-3 weights $Q = \mathrm{diag}(6 \times 10^4 I_3,\; 60\, I_3)$, $R = 0.01 I_3$ render acceleration-level gains $(K_d, D_d) \approx (6 \times 10^4\, I_3,\; 60\, I_3)$ (their Riccati image, not the weights themselves); at the nominal arm configuration ($\Lambda_\text{arm} \approx 0.20\, I_3\,\text{kg}$) these yield effective impedance stiffness $K_\text{eff} = \Lambda_\text{arm} K_d \approx 1.2 \times 10^4\,\text{N/m}$ and effective damping $D_\text{eff} = \Lambda_\text{arm} D_d \approx 12\,\text{Ns/m}$, consistent with compliant pHRI operation.
...
## IX. Complete Torque Equation and Architectural Comparison

### A. Complete Joint Torque Equation
...
| Label | Description |
| :--- | :--- |
| D1 | Operational-space PD (task PD, no priority hierarchy), $K_P = 800\,\text{N/m}$, $K_D = 40\,\text{Ns/m}$ |
...
| D7 Proposed full | **0.43** | **0.40** |

![Fig. 1](simulation/results/scenario_a_results.png)

**Fig. 1.** Scenario A — Fixed double-support stance, 8 N step pHRI disturbance at $t = 0.5\,\text{s}$. *Top:* Cartesian end-effector error norm $\|e\|$ over time for all seven controllers. In static double support all predictive controllers — including the fixed-base D3 — reach sub-millimetre steady state within $\sim$0.3 s of disturbance onset; contact-consistency does not distinguish them here (its value appears only under dynamic loading, Scenario B). The reactive baselines D1/D4 plateau at the $\sim$10 mm impedance offset and D2 (OS PI) at $\sim$5.9 mm. *Bottom:* Bar chart of RMS and steady-state (SS, t > 3.5 s) error; D6/D7 reach 0.43 mm RMS / 0.40 mm SS (Table III).

Key findings. (1) The reactive baselines (D1, D4) leave the theoretical 10 mm impedance offset ($8\,\text{N}/800\,\text{N/m}$). (2) In static double support **all** predictive controllers reach sub-millimetre steady state — including the fixed-base D3 (0.15 mm): with sufficient feedback gain the offset is suppressed regardless of the inertia model, because a static stance never excites the contact-mode dynamics. Contact-consistency is therefore *not* what drives static precision; its value appears only under dynamic loading (Scenario B). (3) The Kalman gives a modest steady-state improvement (D5 0.74 → D7 0.40 mm, 1.8×), and covariance inflation has no effect (D6 = D7) because the contact mode never switches in this scenario.
...
| D2 OS PI | 5.91 | 10.17 |
| D3 Fixed-base MPC | **372.3** | **387.9** |
| D4 WBC + PD | 11.89 | 17.24 |
| D5 Proposed, no Kalman | 0.82 | 1.25 |
| D6 Proposed, no inflation | 0.50 | 0.87 |
| D7 Proposed full | **0.50** | **0.86** |

![Fig. 2](simulation/results/scenario_b_results.png)

**Fig. 2.** Scenario B — double-support stance, sustained 8 N pHRI + 6 N periodic shocks at 1 Hz. This is the decisive test: *Top:* Error norm over the full episode. The fixed-base D3 (which is among the best controllers in the static Scenario A) **diverges** under the periodic excitation — its free-space inertia model under-estimates the contact-coupled apparent inertia at the hand (~5 vs ~1.2 kg), producing an over-aggressive predictive gain (RMS 372 mm). The contact-consistent controllers D5–D7 stay stable and sub-millimetre (peak $\leq$1.25 mm at each shock), because $\Lambda_\text{arm}$ correctly reflects the higher apparent inertia. D1/D4 (OS PD/WBC-PD) show the periodic spike train (peak 15.77 mm) and D2 (OS PI) shows integral windup at each step-change (peak 10.17 mm). *Bottom:* Bar chart of RMS and peak-at-transition error. This scenario is where contact-consistency earns its role — not in static precision, but in stability under dynamic contact loading.

This scenario is the decisive test. Under the periodic shocks the fixed-base D3 — among the best controllers in the static Scenario A — **diverges** (RMS 372 mm): its free-space inertia under-estimates the true contact-coupled apparent inertia at the hand (~5 vs. ~1.2 kg), producing an over-aggressive predictive gain that the dynamic excitation destabilizes. The contact-consistent controllers (D5–D7) remain stable and sub-millimetre (RMS ≤ 0.82 mm) because $\Lambda_\text{arm}$ correctly reflects the higher apparent inertia and hence a well-conditioned gain. **This is where contact-consistency earns its role** — not in static precision, but in stability under dynamic contact loading. The Kalman again yields a modest gain (D5 0.82 → D7 0.50 mm). Covariance inflation is inert here (D6 = D7): despite the label, this scenario applies force spikes *without* an actual foot lift-off or touchdown, so the contact mode never changes and there is nothing for inflation to re-estimate. A genuine test of the inflation protocol requires a real contact-set change, provided by the bracing-hand transition of Scenario E.
...
| D7 Proposed full | **1.50** | **2.41** |

![Fig. 3](simulation/results/scenario_c_g1_results.png)

**Fig. 3.** Scenario C — Unitree G1 official model (33.3 kg, 29 DOF), fixed stance, 8 N step pHRI. *Top:* In static G1 stance the contact-consistent (D7, 2.41 mm SS) and fixed-base (D3, 2.28 mm SS) predictive controllers are comparable, and all predictive controllers improve $\sim$4× over the reactive D1 (9.70 mm SS); the residual few-millimetre error (vs. sub-millimetre on the biped) reflects the position-as-torque bandwidth limitation — with $K_p = 500$ and joint inertia $\sim$0.5 kg⋅m$^2$, the arm actuators reach only $\sim$5 Hz, partially attenuating fast MPC corrections. *Bottom:* RMS and SS error bar chart (D4–D6 reported in Table V).

Key findings. As in Scenario A, the static G1 stance does not distinguish contact-consistent from fixed-base predictive control: D3 (2.28 mm) and D7 (2.41 mm) are comparable, and all predictive controllers improve ~4× over the reactive D1 (9.70 mm). The Kalman benefit is negligible in this static case (D5 2.45 → D7 2.41 mm). The residual few-millimetre error (vs. sub-millimetre on the biped) arises from the position-actuator bandwidth (~5 Hz with $K_p = 500$) attenuating the 1 kHz MPC corrections; direct joint-torque mode on G1/R1 EDU ($K_p = K_d = 0$, $\tau_\text{ff} \neq 0$) would recover it.
...
| D7 Kalman + inflation ($\alpha=4$) | **2.70** | **4.42** |

This scenario validates the covariance-inflation protocol. At each contact-mode switch the Kalman input matrix $B_d^{(m)}$ jumps discontinuously; without inflation (D6) the stale covariance briefly biases $\hat{d}$, so the peak transition error (4.67 mm) is in fact *worse* than the no-Kalman controller (D5, 4.50 mm). Inflating the covariance at the switch (D7, $\alpha=4$) widens the filter, accelerates re-estimation of $\hat{d}$ under the new $B_d$, and recovers the lowest peak (4.42 mm). The effect is modest in magnitude but confirms the protocol's mechanism on a genuine contact-set change. Scenario F extends this to a genuine single↔double-support transition.

### G. Scenario F: Quasi-Static Single↔Double Support Transition

Scenarios B and E hold both feet planted; this scenario exercises a genuine change of the **foot** support set — the biped shifts its weight over the right foot, lifts the left foot into single support, holds, and places it back:

$$\{\text{L foot}, \text{R foot}\} \;\longleftrightarrow\; \{\text{R foot}\},$$

so the contact Jacobian ($J_c$: 6↔3 rows), $\bar{M}$, the arm task inertia $\Lambda_\text{arm}$, and the input matrix $B_d$ all switch at the lift and the place. The interaction layer runs above the balance controller with a sustained 8 N pHRI force on the arm; the arm regulates a target fixed relative to the torso, isolating its disturbance-rejection task from the balancing motion.

**Balance stand-in and its limitations.** A standing single-support phase requires **ankle-roll** (lateral centre-of-pressure) authority, which the biped of Scenarios A/B/E lacks; hip-roll balancing alone produces a foot-tipping moment that cannot be countered, and the robot falls. We therefore use a modified model (`biped_qstatic`: a wider 18 cm foot and added ankle-roll actuators) and a hand-tuned quasi-static controller — a stiff hip-roll CoM regulator for the coarse weight transfer plus a centre-of-pressure–limited ankle-roll torque for the fine single-support stabilization. This is a deliberate **minimal stand-in** for the Level-1 balance layer, *not* the centroidal MPC of Section V; it demonstrates that the transition is achievable and that the interaction layer stays stable across it, but the balance numbers are not a contribution of this paper.

**Table VII: Scenario F — Quasi-Static Single↔Double Support Transition, Sustained 8 N pHRI (torso-relative arm error)**

| Controller | RMS err [mm] | Peak at switch [mm] |
| :--- | :---: | :---: |
| D5 Proposed, no Kalman | 2.71 | 5.59 |
| D6 Kalman, no inflation ($\alpha=1$) | 2.45 | 4.84 |
| D7 Kalman + inflation ($\alpha=4$) | **2.44** | **4.83** |

The interaction layer keeps the arm sub-few-millimetre and the biped stays upright across the whole cycle (minimum torso height 0.86 m; single-support interval ≈2.1 s). The arm task inertia genuinely switches with the foot-contact set — $\Lambda_\text{arm}$ diagonal $[2.05, 1.66, 0.32]$ kg in double support versus $[1.61, 1.50, 0.27]$ kg in single support (~21% change on the dominant axis) — so this is the first scenario in which a **leg** contact-set change, rather than a hand brace, drives the contact-mode switch and the $B_d^{(m)}$ selection.

The Kalman augmentation again gives a modest improvement, visible both in RMS (D5 2.71 → D7 2.44 mm) and in the peak at the switch (D5 5.59 → D6 4.84 mm). Covariance inflation, however, is **neutral** (D6 4.84 vs D7 4.83 mm): unlike the planted-foot Scenario E, the peak error at a support transition is dominated by the whole-body balancing motion — the torso adjusting under the newly single foot — rather than by staleness of the disturbance estimate, so faster Kalman re-convergence has little to correct. The honest reading across Scenarios E and F is that the inflation protocol behaves as designed but its benefit is small and situational.

![Fig. 4](simulation/results/scenario_f_results.png)

**Fig. 4.** Scenario F — torso-relative arm error across a quasi-static single↔double support transition (shaded: single support). All controllers stay stable across the genuine foot contact-set switch; the Kalman gives a modest offset improvement and covariance inflation is neutral (Table VII).

---

## XI. Conclusion

This paper reframed safe whole-body interaction as a *model-based interaction-dynamics* problem and developed the corresponding *interaction layer* of a floating-base whole-body controller: a null-space Impedance MPC that rejects pHRI disturbances at the arm end-effector with zero steady-state error at fixed contact mode, within the residual null space of the balance tasks. Its decisive benefit is **stability under dynamic contact loading**: a fixed-base impedance MPC of comparable gain—indistinguishable from ours in static stance—diverges under periodic contact shocks (RMS 372 mm), while the contact-consistent layer stays stable and sub-millimetre (0.82 mm), because the task inertia $\Lambda_\text{arm}$ reflects the true contact-coupled apparent inertia and hence a well-conditioned predictive gain. This is a physically grounded and reproducible margin, in contrast to steady-state-error reductions at a fixed configuration, which we find are gain-dominated rather than a unique property of the disturbance observer. The surrounding balance hierarchy—a centroidal MPC and a priority-driven contact-consistent WBC—is the architecture into which the layer is designed to plug; the simulations here run it above a stabilizing balance controller, and closing the loop with the full balance stack is a primary direction for future work. The central technical contribution enabling this is the proof that a contact-consistent feedback linearization reduces the arm end-effector plant to a linear double integrator with a constant discrete state matrix within each contact mode, enabling offline precomputation of the QP cost inverse and ≥1 kHz operation. The contact-mode-indexed input-matrix library and a Kalman covariance-inflation protocol carry these properties across contact transitions. The inflation protocol is validated on the bracing-hand support transition of Scenario E, where the active contact set genuinely switches while both feet stay planted: inflation recovers the lowest peak transition error, and without it the Kalman is briefly worse than the no-Kalman controller at each switch. Scenario F further demonstrates the interaction layer running across a genuine single↔double-support transition; there, however, a hand-tuned quasi-static balance stand-in (with added ankle-roll authority) substitutes for the Level-1 balance layer, and closing the loop with the full centroidal-MPC balance controller of Section V — so that a *dynamic* support transition is stabilized without the modelling concessions — remains future work. The Impedance Equivalence Theorem establishes formal equivalence with classical operational-space impedance control in the infinite-horizon limit, facilitating impedance parameter design.

The proposed architecture occupies a structural niche not addressed by prior locomotion-centric frameworks [3]–[6]: it deliberately halts the WBC stack after balance constraints are satisfied and injects a predictive, compliance-controlled manipulation layer into the residual null space.

More broadly, this result establishes floating-base robots as the second setting—after fixed-base manipulators [23]—in which safe physical interaction reduces to the *same* constant-model predictive interaction plant, with contact switching entering only through the input matrix. We view this configuration-normalized interaction-dynamics reduction, rather than the three-level architecture that implements it, as the transferable contribution: it suggests a common template in which each new platform (fixed base, floating base, and beyond) is handled by deriving its interaction dynamics and showing that they normalize to the same predictive model.
...
[23] Y. Cao and J. Tang, "Impedance MPC for physical human–robot interaction: Predictive disturbance rejection with joint-limit safety," *arXiv preprint* arXiv:2606.08281, 2026.
